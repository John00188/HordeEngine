...
local function setint(name, v)
  local c = GetConVar(name)
  if c then c:SetInt(v) end
end
local function getfloat(name, def)
  local c = GetConVar(name)
  return c and c:GetFloat() or def
end

-- perfmesh_quiet [0|1]  (no arg prints current)
...
-- perfmesh_info  (print a single-line summary)
...
-- pmp  (cycle presets: balanced -> aggressive -> cinematic)
...
-- perfmesh_demo_showcase  (short, safe demo sequence)
